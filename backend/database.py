...
"""
from sqlalchemy import Column, Integer, String, Numeric, Date, ForeignKey, Boolean, UniqueConstraint, create_engine
...
class DailyEntry(Base):
    ...
    labor_entries = relationship("LaborEntry", back_populates="daily_entry", cascade="all, delete-orphan")
...
class LaborEntry(Base):
    """Labor entry for a daily entry"""
    __tablename__ = "labor_entries"
    __table_args__ = (
        UniqueConstraint('daily_entry_id', 'labor_role_id', name='unique_entry_labor'),
    )
    ...
    def to_dict(self):
        return {
            ...
            "straight_rate": float(self.labor_role.straight_rate) + (2.00 if self.night_shift else 0),
            "overtime_rate": float(self.labor_role.overtime_rate) + (2.00 if self.night_shift else 0),
            "total_amount": self.total_amount
        }
