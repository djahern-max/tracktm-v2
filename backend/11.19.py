"""
Bulk Import Timesheets - TrackTM
Simple format for importing multiple days of timesheet data
"""
    ...
    try:
        ...
        if "labor" in entry_data:
            ...
            for labor in entry_data["labor"]:
                ...
                labor_total += cost
                print(
                    f"  [OK] {labor.get('employee_name', 'Unknown')}: {labor.get('regular_hours', 0)} reg hrs = ${cost:,.2f}"
                )
        ...
        if "equipment" in entry_data:
            ...
            for equip in entry_data["equipment"]:
                ...
                # Determine rate based on period
...
def import_all_entries(entries):
    """Import multiple entries"""
    print("\n" + "=" * 60)
    print("BULK TIMESHEET IMPORT")
    print("=" * 60)
    ...
    for entry in entries:
        if import_timesheet_entry(entry):
            successful += 1
        else:
            failed += 1
...
# ============================================
# DEFINE YOUR TIMESHEET ENTRIES HERE
# ============================================

TIMESHEET_ENTRIES = [
    # ============================================
    # Day 2: Wednesday 11/19/2025
    # 6 workers × 11 hours each = 66 labor hours
    # Equipment: Ford F250, PJ Trailer, Graco Extreme
    # ============================================
    {
        "job_number": "2507",
        "date": "2025-11-19",
        # LABOR - List all workers with their hours
        # Available roles: "Supervisor", "Painter", "Project Manager", "Per Diem"
        "labor": [
            {
                "role": "Painter",
                ...
                "regular_hours": 11,
                "overtime_hours": 0,
                "night_shift": False,  # Add $2/hr differential if True
            },
            {
                "role": "Painter",
        ...
        # MATERIALS - Must match exact names in materials catalog
        # Use: SELECT name FROM materials ORDER BY category, name;
        "materials": [
            {
                "name": "Acrolon Paint",
                "quantity": 15,
            },  # CORRECTED: Was 110, should be 15
            {"name": "Organic Vapor Cartridge 6001", "quantity": 3},
            {"name": "Coveralls 2XL", "quantity": 5},
            {"name": "Gloves Thermal", "quantity": 5},
            {"name": "P100 Filter Particulate", "quantity": 3},
            {"name": "Bucket Liners 5 Gallon", "quantity": 15},
            {"name": "White Tape (Eagle)", "quantity": 4},
            {"name": "Drop Cloth 4x15", "quantity": 10},
            {"name": "Rags", "quantity": 1},
        ],
        # EQUIPMENT - Must match exact names in equipment_rental_rates
        # Use: SELECT name, daily_rate FROM equipment_rental_rates WHERE active = 1;
        "equipment": [
    ...
    #
    #     "labor": [
    #         {"role": "Painter", "employee_name": "Worker Name", "regular_hours": 10, "overtime_hours": 0},
    #     ],
    #
    #     "materials": [
    #         {"name": "Material Name", "quantity": 5},
    #     ],
    #
    #     "equipment": [
    #         {"name": "Equipment Name", "quantity": 1, "rate_period": "daily"},
    #     ],
    # },
]
...
if __name__ == "__main__":
    import_all_entries(TIMESHEET_ENTRIES)
